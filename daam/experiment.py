...
COCO80_TO_27 = {
    ...
    "fire hydrant": "accessory",
    "stop sign": "accessory",
    "parking meter": "accessory",
    "bench": "furniture",
    "bird": "animal",
    "cat": "animal",
    "dog": "animal",
...
def build_word_list_coco80() -> Dict[str, List[str]]:
    ...
    words_map = {
        ...
        if not any(item in COCO80_ONTOLOGY for item in v)
    }
...
class GenerationExperiment:
    ...
    def save(
        self, path: str = None, heat_maps: bool = True, tokenizer: AutoTokenizer = None
    ):
        if path is None:
            path = self.path
        else:
    ...
    def _load_pred_masks(
        self, pred_prefix, composite=False, simplify80=False, vocab=None
    ):
        ...
        if composite:
            ...
            for mask_path in (self.path / self.subtype).glob(
                f"*.{pred_prefix}.pred.png"
            ):
                mask = load_mask(str(mask_path))
    ...
    def save_heat_map(
        ...
        with auto_autocast(dtype=torch.float32):
            path = (
                self.path / self.subtype / f"{output_prefix}{word.lower()}.heat_map.png"
            )
    ...
    def save_all_heat_maps(
        self, tokenizer: PreTrainedTokenizer = None, crop: int = None
    ) -> Dict[str, Path]:
        path_map = {}
