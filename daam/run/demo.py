...
def get_args():
    ...
    parser.add_argument(
        "--port", "-p", type=int, default=8080, help="the port to launch the demo"
    )
    parser.add_argument(
        "--no-cuda", action="store_true", help="Use CPUs instead of GPUs"
    )
...
def main():
    ...
    pipe = StableDiffusionPipeline.from_pretrained(args.model, use_auth_token=True).to(
        device
    )
    lock = Lock()
    ...
    def plot(prompt, choice, replaced_word, inf_steps, is_random_seed):
        ...
        if choice:
            ...
            new_prompt = [
                replaced_word if tok.text == choice else tok.text
                for tok in cached_nlp(prompt)
            ]
            ...
            if choice:
                ...
                if new_prompt == prompt:
                    ...
                    with trace(pipe, load_heads=True) as tc:
                        out2 = pipe(
                            new_prompt, num_inference_steps=inf_steps, generator=gen
                        )
        ...
        fig_soft, axs_soft = plt.subplots(
            math.ceil(len(words) / num_cells), num_cells, figsize=(w, h)
        )
        ...
        with gr.Row():
            with gr.Column():
                dropdown = gr.Dropdown(
                    ...
                    label="Examples",
                    value="An angry, bald man doing research",
                )

                text = gr.Textbox(
                    label="Prompt", value="An angry, bald man doing research"
                )
                ...
                with gr.Row():
                    ...
                    dropdown2 = gr.Dropdown(
                        tokens, label="Adjective to replace", interactive=True
                    )
                    text2 = gr.Textbox(label="New adjective", value="")

                checkbox = gr.Checkbox(value=False, label="Random seed")
                slider1 = gr.Slider(
                    15, 30, value=25, interactive=True, step=1, label="Inference steps"
                )

                submit_btn = gr.Button("Submit", elem_id="submit-btn")
                viz = gr.HTML(
                    dependency("An angry, bald man doing research"), elem_id="viz"
                )
