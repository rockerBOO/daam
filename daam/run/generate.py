...
def main():
    ...
    parser.add_argument(
        "--model", type=str, default="v2-1-base", choices=list(model_id_map.keys())
    )
    ...
    if args.action.startswith("coco"):
        ...
        for exp_folder in Path(args.input_folder).iterdir():
            if (
                not GenerationExperiment.contains_truth_mask(exp_folder)
                and args.truth_only
            ):
                continue
        ...
        for gen_idx, (prompt_id, prompt) in enumerate(tqdm(prompts)):
            ...
            with trace(
                ...
                for word in prompt.split():
                    if (
                        args.lemma is not None
                        and cached_nlp(word)[0].lemma_.lower() != args.lemma
                    ):
                        continue
                        ...
                        for head_idx in range(16):
                            for layer_idx, layer_name in enumerate(tc.layer_names):
                                try:
                                    ...
                                    print(
                                        f"Missing ({layer_idx}, {head_idx}, {layer_name})"
                                    )
